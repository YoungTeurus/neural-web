...
class LinearBinaryClassification(Task):
    '''
    ...
    def generateInput(cls) -> list[tuple[NeuralWebInput, NeuralWebOutput]]:
        return LinearBinaryClassification.generateInputOptions()

    @classmethod
    def generateInputOptions(cls, options: Options = None) -> list[tuple[NeuralWebInput, NeuralWebOutput]]:
        _options = options if options is not None else LinearBinaryClassification.Options.default()
        ...
        for i in range(amountOfPoints):
            ...
            y = random.random()
            point = [x, y, 1]
            _class = LinearBinaryClassification.getPointClass(options, point)

            inAndOut.append((point, [_class]))

        return inAndOut

    @classmethod
    def getPointClass(cls, options: 'LinearBinaryClassification.Options', point: list[float]) -> int:
